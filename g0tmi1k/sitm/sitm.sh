...
function action() { #action title command #screen&file #x|y|lines #hold
   ...
   if [ "$error" == "free" ]; then
      ...
      $xterm -geometry 87x$lines+$x+$y -T "sitm v$version - $1" -e "$command"
      return 0
   else
...
#    fi
# fi
# if [ ! -e "/usr/local/bin/ettercap" ]; then
